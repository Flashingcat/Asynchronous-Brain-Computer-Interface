...
该结果说明单窗 argmax 状态机能显著减少空闲期误输出，但会漏掉较多事件；它是后续多窗聚合、置信度阈值和复位规则的最低对照，不代表最终系统。当前结果位于 `results/tables/s01_s09_epoch50_causal_single_window_oof_precommit_v2`，运行清单标记为 `PRECOMMIT_DIAGNOSTIC`；只有源码审核、用户审批、Git 提交及干净 commit 上的等价重跑全部完成后，才能升级为正式候选。
