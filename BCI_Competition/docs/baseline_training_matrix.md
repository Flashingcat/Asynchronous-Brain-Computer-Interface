...
python BCI_Competition/code/eval/run_hard_vote_matrix.py `
  ...
  --output-root BCI_Competition/results/tables/s01_s09_epoch50_causal_hard_vote_matrix_precommit_v4
```
...
预提交运行目录为 `results/tables/s01_s09_epoch50_causal_hard_vote_matrix_precommit_v4`，状态为 `PRECOMMIT_DIAGNOSTIC_MATRIX`。每个被试和 seed 均保存联合硬标签、八组完整状态/输出轨迹和逐事件指标；顶层同时保存包含单窗锚点的逐 seed 与跨 seed CSV。顶层及各被试清单还记录实际 Python 解释器、环境目录、Python/NumPy 版本、主机和平台，环境身份不依赖外部聊天记录。提交本轮代码后可从干净 commit 重跑为 `CLEAN_COMMIT_DIAGNOSTIC_MATRIX`，但“诊断矩阵”身份不会因此自动变成已选择的正式工作点。
